...
- **Window setup:** `.nonactivatingPanel` style mask + a status-bar-appropriate window level, so opening the panel never steals focus/activation from the frontmost app (a hard requirement for a menu-bar utility). Implemented via `orderFrontRegardless()`, never `makeKeyAndOrderFront` — the panel never becomes key. This has a real consequence caught during implementation: AppKit's default `acceptsFirstMouse(for:)` is `false`, so a permanently-non-key panel needs an explicit `NSHostingView` subclass overriding it to `true`, or every click inside the panel (Join, an agenda row) would be swallowed just to activate the window, requiring a second click to register.
- **Dismissal triggers:** click outside the panel (global event monitor), Esc key, re-clicking the status item, the app resigning key status, and system screen sleep and lock — sleep via `NSWorkspace`, lock via the same mechanism `LifecycleObserver` already uses elsewhere in the app (reused, not duplicated). Pressing "Join" dismisses the panel after opening the link, mirroring `NSMenu`'s dismiss-on-item-click behavior. **Known open risk:** because the panel never becomes key, Esc can only be caught with a *global* event monitor (a local one only sees events sent to this app, and Esc while another app is frontmost isn't one) — but global monitors for keyboard events (unlike mouse events) require the user to have granted Input Monitoring permission, which this app doesn't request. Esc-to-dismiss may silently do nothing on a typical first run; the other four triggers don't have this dependency and stay reliable. Needs a real-device check to resolve (prompt for the permission, or accept Esc as best-effort).
- **Positioning:** anchored under the status item, clamped to stay on-screen near display edges or a crowded/notched menu bar (both horizontal and vertical edges).
- **Left-click vs. right-click:** the plan originally assumed right-click already showed the classic `NSMenu` and could keep doing so unchanged. It doesn't — right-click is `joinNextMeeting()`, an existing instant-join shortcut with no menu involved at all (confirmed by reading `StatusBarItemController.statusMenuBarAction`). Overriding it for the classic menu would have silently dropped that shortcut. Fix: right-click stays untouched; left-click shows the new panel (replacing its old `openMenu()` call); the classic menu is reached instead via a "More…" row inside the panel. Also found: the status item button's `sendAction(on:)` fires on both mouse-down and mouse-up per left click, which the old blocking `NSMenu` tracking loop tolerated but the new non-blocking panel show/toggle doesn't — the click handler now only acts on mouse-up to avoid opening and immediately closing the panel on every click.
...
- The panel implementation (Panel mechanics, above) was written and code-reviewed without access to full Xcode, verified only via ad-hoc `swiftc`/throwaway-SPM-package compiles against the real SDK and dependencies — real, but compile-time only, not click-tested. Two interaction bugs (a left-click double-fire, a dead Esc path) were already caught this way and fixed; the Input Monitoring gap on Esc was caught but not yet resolvable without a device. Treat this UI as unverified-by-running until the first real `xcodebuild`/on-device pass, which should specifically re-check: first-click responsiveness (the `acceptsFirstMouse` fix), all five dismissal triggers, and the live-countdown refresh.
